...
//             return s;
          ...
          if (tree)
          {
             ...
             fprintf(fp, "if [ -f $IMAGE./%s%s ]; then\n", 
                     dir1, blp);
             ...
             if (cnvt)          
             {
                fprintf(fp, "elif [ -f $IMAGE./%s%s ]; then\n", 
                     dir2, bulp);
                fprintf(fp, "\t/bin/mkdir -p $OUTPUT./%s\n", dir2);
                fprintf(fp, "\tcp -f $IMAGE./%s%s $OUTPUT./%s%s\n", 
                     dir2, bulp, dir2, bulp);
          ...
          else
          {
             ...
             fprintf(fp, "\tcurl --retry 1 -f -O $IMAGEPATH./%s%s\n",
                  dir1, blp);
             fprintf(fp, "\tif [ -f $IMAGE./%s ]; then\n", blp);
             fprintf(fp, "\t\t/bin/mkdir -p $OUTPUT./%s\n", dir1);
             ...
             fprintf(fp, "\telse\n");
             fprintf(fp, "\t\tcurl --retry 1 -f -O $COMMONSPATH./%s%s\n",
                  dir1, blp);
...
int main(int argc, char *argv[])
{
    ...
    iPath[0] = '\0';
    cPath[0] = '\0';
    strcpy(iPath, "http://upload.wikimedia.org/wikipedia/de/");
